...
export default function Projects() {
  ...
  return (
    <section id="projects" className="relative min-h-screen text-white px-6 md:px-24 py-24 md:py-32 overflow-hidden" style={{background: "radial-gradient(ellipse at 40% 70%, #050d1f 0%, #000000 70%)"}}>
      ...
      <div className="relative z-10">
        <p className="text-[#2563FF] tracking-[0.3em] mb-8 text-xs md:text-sm font-medium">PROJECTS</p>
        <h2 className="text-4xl md:text-6xl font-bold mb-12 md:mb-20 text-white">Selected Work.</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-10">
          {projects.map((project) => (
            <div key={project.title} className="rounded-3xl p-6 md:p-8 transition-all duration-300 hover:scale-105 group flex flex-col justify-between" style={{border: "1px solid rgba(37,99,255,0.2)", background: "rgba(37,99,255,0.04)", boxShadow: "0 0 30px rgba(37,99,255,0.05)"}}>
              <div>
                <span className="text-xs tracking-widest text-[#2563FF] uppercase mb-4 block">{project.tag}</span>
                <h3 className="text-2xl md:text-3xl font-bold mb-4 text-white group-hover:text-[#2563FF] transition-colors duration-300">{project.title}</h3>
                <p className="text-[#94A3B8] leading-relaxed text-sm md:text-base">{project.description}</p>
              </div>
